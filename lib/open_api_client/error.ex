defmodule OpenAPIClient.Error do
  @moduledoc """
  ...
  ## Fields
    ...
    * `conn` (`t:Plug.Conn.t/0`): `Plug.Conn` at the time of the error.
    ...
    * `plug` (`t:OpenAPIClient.plug/0`): `Plug` active at the time of the error.
  ...
  @type new_option ::
          ...
          | {:reason, atom()}
          | {:source, term()}
          | {:plug, OpenAPIClient.plug()}

  @derive {Inspect, except: [:conn, :stacktrace]}
  defexception [:code, :message, :conn, :reason, :source, :stacktrace, :plug]
  ...
  the status `code` (if available), a `message`, the original `conn`, the `source` of the
  error, and which `plug` or plugin is currently active.
  """
